...
By focusing the energy delivered by ultrasound at different spots in the brain, we were able to trigger different smell-like sensations. We reproduced the effect on our first try with two people and confirmed it in a blind trial. The sensations we identified were:
<SmellList />
...
Smells are processed in the olfactory bulb. We decided to try to stimulate it with focused ultrasound through the skull. As far as we know, *no one seems to have done this kind of olfactory stimulation before* - not even in animals.  
